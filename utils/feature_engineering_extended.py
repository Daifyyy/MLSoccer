...
def generate_extended_features(df):
    ...
    df['xg_home_last5'] = df.groupby('HomeTeam')['home_xg'].transform(lambda x: x.shift().rolling(window=5, min_periods=1).mean())
    df['xg_away_last5'] = df.groupby('AwayTeam')['away_xg'].transform(lambda x: x.shift().rolling(window=5, min_periods=1).mean())

    # Nudný zápas skóre (méně střel a gólů)
    df['boring_match_score'] = (
        (df['HS'] + df['AS']) * 0.05 + (df['HST'] + df['AST']) * 0.1 + (df['FTHG'] + df['FTAG']) * 0.3
    )
